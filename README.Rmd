...
First of all, please install R on your machine (click for [instructions](https://cran.r-project.org/)). In case R is already installed, later versions will have fewer compatibility issues.
...
On mac and windows, download the file [**DAFi.jar**](https://github.com/PedroMilanezAlmeida/DAFi/raw/master/DAFi.jar) and copy the file to FlowJo's plugin folder. More information on how to install a plugin in FlowJo can be found [here](http://docs.flowjo.com/d2/plugins/installing-plugins/).
...
After hitting OK, the plugin runs for a few seconds (about 10-20sec for default number of clusters; the first run may take a few minutes to install packages; when running on a Mac for the firs time, the user will need to instal Xcode and gfortran [see installation instructions above]).
